...
class DifficultyAdjustmentSpecification15 extends ErgoCorePropertyTest {
  ...
  property("eip37Calculate()") {
    forAll(Gen.choose(UseLastEpochs, 10 * UseLastEpochs), defaultHeaderGen) { (startEpoch: Int, header: Header) =>
      whenever(header.requiredDifficulty > 10) {
        ...
        control.eip37Calculate(previousHeaders, Epoch) < header.requiredDifficulty shouldBe true
      }
    }
  }
