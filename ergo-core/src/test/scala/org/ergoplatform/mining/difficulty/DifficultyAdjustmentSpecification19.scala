...
class DifficultyAdjustmentSpecification19 extends ErgoCorePropertyTest {
  ...
  property("eip37Calculate()") {
    ...
    forAll(defaultHeaderGen, smallPositiveInt, smallPositiveInt, Gen.choose(1, 60 * 60 * 1000)) { (header: Header, epoch, useLastEpochs, interval) =>
      whenever(useLastEpochs > 1) {
        ...
        val error = BigDecimal(control.eip37Calculate(previousHeaders, epoch) - expectedDifficulty) / BigDecimal(expectedDifficulty)
